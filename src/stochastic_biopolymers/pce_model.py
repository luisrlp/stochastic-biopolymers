...
class PCEConstructor:
    ...
    def train_pce(self, 
        ...
        if 'STRETCH' not in self.df.columns:
            ...
            pce_dict = {
                ...
                poly_basis,
                self.x(split='train', deformation=deformation),
                self.y(split='train', deformation=deformation),
    ...
    def test_pce(self, pce_dict: dict, 
        ...
        if deformation is not None or len(pce_dict.keys()) == 1:
            ...
            for deformation_value, pce_model in pce.items():
                ...
                mean_squared_error = np.mean(((pce_output - y_val) / y_val) ** 2)
    ...
    def save_pce(self, pce: dict, 
        ...
        deformation = pce.keys()
        if isinstance(deformation, (np.ndarray, list)) and len(deformation) > 1:
            deformation = deformation[0]
            n_train_samples = len(self.df[(self.df['split'] == 'train') & (self.df['deformation'] == deformation)])
        else:
            n_train_samples = len(self.df[self.df['split'] == 'train'])
